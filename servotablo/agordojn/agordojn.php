...
function displayGordojn()
{
    ...
    $displayGordojn = $langStrings['displayGordojn'];

    $displayGordojn_array = getLangstringsArray('displayagordojn_array', $displayLang);
    ...
    $sql = "SELECT * FROM (SELECT node.settingId, node.lft, node.rgt, node.tableKey as masterTableKey, CAST(AES_DECRYPT(node.setting, SHA2('".$phrase."', 512)) as char) as setting, CAST(AES_DECRYPT(node.data, SHA2('".$phrase."', 512)) as char) as data, COUNT(parent.lft) - 1 as depth
        FROM ".$table." AS node,
              ".$table." AS parent
        ...
        ORDER BY node.lft) as menu LEFT OUTER JOIN (SELECT * FROM (SELECT * FROM(SELECT settingId, tableKey, 
              ...
              ) AS lang ON menu.settingId = lang.settingId GROUP BY menu.settingId ORDER BY menu.lft";
     ...
     echo "<div class = \"row\">";
        echo "<div class = \"col-md-3\" style = \"max-height : 95vh; overflow : auto;\">";
            ...
            echo "<form id = \"addForm_".$replaceTable[PREFIX."site_settings"]."\">";
                echo $displayGordojn[1]."<br>";
                echo "<input type = \"text\" id = \"note\" class = \"form-control\"><br>";

                echo "<button type = \"button\" class = \"btn btn-secondary addToTree\" data-target_tree = \"tree_".$replaceTable[PREFIX."site_settings"]."\" data-replace_table = \"".$replaceTable[PREFIX."site_settings"]."\" data-replace_lang = \"".$replaceLang[$lang]."\">".$displayGordojn[2]."</button>";
            echo "</form><br><br>";
...
function displayBodyAgordojn()
{
    ...
    $langStrings = getlangstrings();
    $displayBodyAgordojn = $langStrings['displayBodyAgordojn'];

    $displayBodyAgordojn_array = getLangstringsArray('displayBodyAgordojn_array', $displayLang);
            ...
            echo "<div class=\"col-md-12\" style = \"height : 99%;\">";
                echo "<div class=\"card\"style = \"height : 99%;\">";
                    echo "<div class=\"card-header\">";
                        echo "<h1>".$displayBodyAgordojn[1]."</h1>";
                echo "</div>";
